...
public class SinhVienNCService {
    ...
    public Vector<MonHocNienChe> danhSachMonHocNienChe() {
        return SerilizeFactory.docFileNienChe("CacMonHocNienChe.db");
    }
//    điểm tổng kết 1 môn công thức (Điểm giữa kì*3 + điểm cuối kì *0.7)/10
//    điểm giữa kì tính theo thang 10, điểm cuối kì tính theo thang 100, tổng kết trên 4 là qua

    public static double tinhDiemTongKetMonNC(double diemGiuaKy, double diemCuoiKy) {
        return (0.3 * diemGiuaKy + 0.7 * diemCuoiKy) / 10;
    }
...
//        
        boolean check = SerilizeFactory.luuFileMonNienChe(vec, "CacMonHocNienChe.db");
        if (check) {
